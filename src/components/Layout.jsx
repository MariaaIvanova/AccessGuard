...
export default function Layout({ children }) {
  ...
  function applyTheme(themeName) {
    ...
    if (themeName === 'dark') {
      ...
      s('--bg', 'radial-gradient(circle at 0% 0%, rgba(255, 208, 225, 0.82), transparent 26%), radial-gradient(circle at 100% 10%, rgba(247, 191, 209, 0.72), transparent 22%), radial-gradient(circle at 80% 100%, rgba(255, 224, 234, 0.86), transparent 26%), linear-gradient(180deg, #fffafd 0%, #fff7fa 52%, #fff1f5 100%)')
      s('--surface', 'rgba(255, 255, 255, 0.52)'); s('--card-bg', 'rgba(255, 255, 255, 0.82)')
      s('--border', 'rgba(223, 150, 176, 0.34)'); s('--text', '#2b2025')
      s('--text-muted', '#8e7881'); s('--text-light', '#c3a7b1')
      s('--input-bg', 'rgba(255, 255, 255, 0.72)'); s('--table-head', 'rgba(255, 244, 248, 0.94)')
      s('--btn-bg', 'linear-gradient(135deg, #dd7fa2, #c9638b)'); s('--btn-color', '#fffafc')
    } else {
    ...
    document.body.style.background = themeName === 'dark' ? '#111' : themeName === 'pink'
    ...
    document.body.style.color = themeName === 'dark' ? '#f0f0f0' : themeName === 'pink' ? '#2b2025' : '#1a1a1a'
  ...
  const nextThemeLabel = theme === 'light' ? 'Тъмен режим' : theme === 'dark' ? 'Розов режим' : 'Светъл режим'
    ...
    <ThemeContext.Provider value={{ dark, theme, toggle: cycleTheme }}>
      <style>{`
        ...
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        @keyframes drift { from { transform: translateY(0px); } to { transform: translateY(-36px); } }

        /* ── Responsive ── */
        .table-scroll { overflow-x: auto; -webkit-overflow-scrolling: touch; }
        .page-main { padding: 28px 28px 40px; }
        .grid-2col { display: grid; grid-template-columns: 1fr 1fr; gap: 16px; }
        .grid-fixed { display: grid; grid-template-columns: minmax(0, 360px) minmax(0, 1fr); gap: 16px; align-items: start; }

        @media (max-width: 640px) {
          .page-main { padding: 16px 14px 32px !important; }
          .grid-2col { grid-template-columns: 1fr !important; }
          .grid-fixed { grid-template-columns: 1fr !important; }
        }
        @media (max-width: 600px) {
          .nav-hamburger { display: flex !important; }
          .nav-name-text { display: none !important; }
        ...
        <header style={{
          ...
          <div style={{ height: '100%', padding: '0 16px', display: 'flex', alignItems: 'center', gap: 12 }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 9, cursor: 'pointer', flexShrink: 0 }} onClick={() => navigate('/dashboard')}>
              <div style={{ width: 32, height: 32, background: isPinkTheme ? 'rgba(255,255,255,0.78)' : 'var(--surface)', borderRadius: 9, display: 'flex', alignItems: 'center', justifyContent: 'center', border: '1px solid var(--border)', boxShadow: isPinkTheme ? '0 10px 20px rgba(163, 92, 120, 0.14)' : 'none', overflow: 'hidden', flexShrink: 0 }}>
