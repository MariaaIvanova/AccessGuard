...
export default function Admin() {
  ...
  useEffect(() => {
    try {
      ...
      if (saved) {
        setMaintEnabled(saved.enabled ?? false)
        setMaintStart(saved.start ?? '23:00')
        setMaintEnd(saved.end ?? '06:00')
      }
    } catch { }
  }, [])
  ...
  useEffect(() => {
    if (door?.id) loadTempCodes(door.id)
  }, [door?.id])

  async function loadTempCodes(doorId) {
  ...
  async function approveUser(id) {
    ...
    await logAction('approve_user', id); loadData()
  }
  async function blockUser(id) {
    await supabase.from('users').update({ status: 'blocked' }).eq('id', id)
    await logAction('block_user', id); loadData()
  }
  async function makeAdmin(id) {
    ...
    await logAction('make_admin', id); loadData()
  }
  ...
  async function saveDoorSettings() {
    ...
    if (error) { setDoorSaveMsg('error:Грешка при запазване.') }
      ...
      await supabase.from('audit_logs').insert({ admin_id: currentUser.id, action: 'door_settings_updated', details: { open_warning_minutes: minutes, auto_close_enabled: doorAutoClose } })
      setDoorSaveMsg('success:Настройките са запазени.')
    }
    setDoorSaving(false)
  }
  ...
  async function generateQR() {
    if (!qrGuest || !qrFrom || !qrUntil) {
      await showAlert({ title: 'Непълни данни', message: 'Въведете име на госта и задайте период.', confirmLabel: 'Разбрах', tone: 'warning' }); return
    }
    if (new Date(qrUntil) <= new Date(qrFrom)) {
      await showAlert({ title: 'Невалиден период', message: '"Валиден до" трябва да е след "Валиден от".', confirmLabel: 'Разбрах', tone: 'warning' }); return
    }
    setQrGenerating(true)
    const qrCode = crypto.randomUUID()
    const { data, error } = await supabase.from('temp_access').insert({
      guest_name: qrGuest,
      door_id: door.id,
      qr_code: qrCode,
      valid_from: new Date(qrFrom).toISOString(),
      valid_until: new Date(qrUntil).toISOString(),
      created_by: currentUser.id,
      is_used: false,
    }).select().single()
    if (!error && data) {
      setQrLatest(data)
      await logAction('temp_access_created', null, { door_id: door.id, guest_name: qrGuest, valid_until: qrUntil })
      loadTempCodes(door.id)
      setQrFrom(''); setQrUntil(''); setQrGuest('')
    }
    setQrGenerating(false)
  }
  async function revokeQR(id) {
    await supabase.from('temp_access').update({ is_used: true }).eq('id', id)
    loadTempCodes(door.id)
  }
    ...
    <Layout>
      <main style={{ padding: '28px 32px 40px', background: 'var(--bg)', minHeight: 'calc(100vh - 56px)' }}>
          ...
          <div>
            ...
            <div style={{ background: 'var(--card-bg)', border: '1px solid var(--border)', borderRadius: 12, overflow: 'hidden' }}>
              <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                <thead><tr style={{ background: 'var(--table-head)' }}>{['Потребител', 'Имейл', 'Роля', 'Статус', 'Действия'].map(h => <th key={h} style={{ fontSize: 11, fontWeight: 500, color: 'var(--text-muted)', textTransform: 'uppercase', letterSpacing: 0.4, padding: '10px 20px', textAlign: 'left', borderBottom: '1px solid var(--border)' }}>{h}</th>)}</tr></thead>
                <tbody>
                    ...
                    <tr key={u.id} style={{ borderBottom: i < filteredUsers.length - 1 ? '1px solid var(--border)' : 'none', opacity: u.is_blacklisted ? 0.55 : 1 }}>
                      ...
                      <td style={{ padding: '12px 20px' }}>
                        <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
                          {u.status === 'pending' && <button onClick={() => approveUser(u.id)} style={{ padding: '4px 10px', background: '#f0fdf4', border: '1px solid #bbf7d0', borderRadius: 6, fontFamily: "'Inter', sans-serif", fontSize: 11, fontWeight: 500, cursor: 'pointer', color: '#16a34a' }}>Одобри</button>}
                          {u.status === 'active' && u.id !== currentUser?.id && <button onClick={() => blockUser(u.id)} style={{ padding: '4px 10px', background: '#fef2f2', border: '1px solid #fecaca', borderRadius: 6, fontFamily: "'Inter', sans-serif", fontSize: 11, fontWeight: 500, cursor: 'pointer', color: '#ef4444' }}>Блокирай</button>}
                          {u.role !== 'admin' && u.id !== currentUser?.id && <button onClick={() => makeAdmin(u.id)} style={{ padding: '4px 10px', background: '#fffbeb', border: '1px solid #fde68a', borderRadius: 6, fontFamily: "'Inter', sans-serif", fontSize: 11, fontWeight: 500, cursor: 'pointer', color: '#92400e' }}>Направи админ</button>}
                          {u.id !== currentUser?.id && <button onClick={() => toggleBlacklist(u.id, u.is_blacklisted)} style={{ padding: '4px 10px', background: u.is_blacklisted ? '#f0fdf4' : '#fef2f2', border: `1px solid ${u.is_blacklisted ? '#bbf7d0' : '#fecaca'}`, borderRadius: 6, fontFamily: "'Inter', sans-serif", fontSize: 11, fontWeight: 500, cursor: 'pointer', color: u.is_blacklisted ? '#16a34a' : '#ef4444' }}>{u.is_blacklisted ? 'Премахни от ЧС' : 'Черен списък'}</button>}
                        </div>
                      </td>
                    </tr>
                  ))}
                    ...
                    <div style={{ flex: 1 }}>
                      ...
                      {request.status === 'pending' ? (
                        <div style={{ marginTop: 14 }}>
                          ...
                          <textarea style={{ ...inp, width: '100%', minHeight: 84, resize: 'vertical', lineHeight: 1.5 }} placeholder="Напр. Приемам заявката. Елате утре в 10:00." value={draft}
        ...
        {tab === 2 && (
          <div>
            {blacklisted.length === 0 ? (
              ...
              <div style={{ background: 'var(--card-bg)', border: '1px solid var(--border)', borderRadius: 12, overflow: 'hidden' }}>
                <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                  <thead><tr style={{ background: 'var(--table-head)' }}>{['Потребител', 'Имейл', 'Статус акаунт', 'Действие'].map(h => <th key={h} style={{ fontSize: 11, fontWeight: 500, color: 'var(--text-muted)', textTransform: 'uppercase', letterSpacing: 0.4, padding: '10px 20px', textAlign: 'left', borderBottom: '1px solid var(--border)' }}>{h}</th>)}</tr></thead>
                  <tbody>
                    {blacklisted.map((u, i) => (
                      <tr key={u.id} style={{ borderBottom: i < blacklisted.length - 1 ? '1px solid var(--border)' : 'none' }}>
                        ...
                        <td style={{ padding: '12px 20px', fontSize: 12, color: 'var(--text-muted)' }}>{u.email}</td>
                        <td style={{ padding: '12px 20px' }}><Badge type={u.status} /></td>
                        <td style={{ padding: '12px 20px' }}><button onClick={() => toggleBlacklist(u.id, true)} style={{ padding: '6px 14px', background: '#f0fdf4', border: '1px solid #bbf7d0', borderRadius: 8, fontFamily: "'Inter', sans-serif", fontSize: 12, fontWeight: 500, cursor: 'pointer', color: '#16a34a' }}>Премахни от черния списък</button></td>
                      </tr>
                    ))}
                  </tbody>
                </table>
        ...
        {tab === 3 && (
          <div style={{ background: 'var(--card-bg)', border: '1px solid var(--border)', borderRadius: 12, overflow: 'hidden' }}>
            <table style={{ width: '100%', borderCollapse: 'collapse' }}>
              <thead><tr style={{ background: 'var(--table-head)' }}>{['Дата и час', 'Администратор', 'Действие'].map(h => <th key={h} style={{ fontSize: 11, fontWeight: 500, color: 'var(--text-muted)', textTransform: 'uppercase', letterSpacing: 0.4, padding: '10px 20px', textAlign: 'left', borderBottom: '1px solid var(--border)' }}>{h}</th>)}</tr></thead>
              <tbody>
                {auditLogs.length === 0
            ...
            {!door ? (
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16, alignItems: 'start' }}>
                ...
                <div style={{ background: 'var(--card-bg)', border: '1px solid var(--border)', borderRadius: 12, padding: '16px 20px' }}>
                  ...
                  {qrLatest && (
                    <div style={{ padding: '14px', background: '#f0fdf4', border: '1px solid #bbf7d0', borderRadius: 10, display: 'flex', gap: 14, alignItems: 'center' }}>
                      ...
                      <div>
                        ...
                        <div style={{ fontSize: 11, color: '#166534', marginBottom: 2 }}>Гост: {qrLatest.guest_name}</div>
                ...
                <div style={{ background: 'var(--card-bg)', border: '1px solid var(--border)', borderRadius: 12, padding: '16px 20px' }}>
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                    {tempCodes.map(tc => {
                      ...
                      return (
                        <div key={tc.id} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '10px 14px', borderRadius: 9, border: '1px solid var(--border)', background: tc.is_used || expired ? 'var(--input-bg)' : 'var(--card-bg)', opacity: tc.is_used || expired ? 0.6 : 1 }}>
                          <div>
                            <div style={{ fontSize: 13, fontWeight: 500, color: 'var(--text)' }}>{tc.guest_name || '—'}</div>
