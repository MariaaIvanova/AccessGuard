...
import { HourlyBarChart, MethodPieChart, GrantedDeniedLineChart, KpiCard } from '../components/Charts'
...
const TABS = ['Потребители', 'Запитвания', 'Черен списък', 'Одит лог', 'Врата', 'Анализ']
const ACTION_LABELS = {
...
// Универсален UUID v4 генератор. crypto.randomUUID() съществува само в secure
// context (HTTPS или localhost), а ние сме на мрежов IP за демо. Затова имаме fallback.
function generateUUID() {
  ...
  // Последна резерва (за съвсем стари браузъри) — не е криптографски, но става за token
...
// Превръща HH:MM:SS (от Postgres time) или HH:MM в HH:MM (за input type="time")
...
export default function Admin() {
  ...
  const [maintStart, setMaintStart] = useState('23:00')
  const [maintEnd, setMaintEnd] = useState('06:00')
  ...
  const loadData = useCallback(async () => {
    ...
    if (doorResult.data) {
      ...
      setMaintEnabled(doorResult.data.maintenance_enabled ?? false)
      setMaintStart(timeToHHMM(doorResult.data.maintenance_start) || '23:00')
      setMaintEnd(timeToHHMM(doorResult.data.maintenance_end) || '06:00')
      setTempCodes(tempAccessData || [])
      setRecentCommands(cmdData || [])
    }
    setLoading(false)
  }, [navigate])
  ...
  useEffect(() => {
    const channel = supabase.channel('admin-door-realtime')
      .on('postgres_changes', { event: 'UPDATE', schema: 'public', table: 'doors' }, (payload) => {
        setDoor(payload.new)
        setMaintEnabled(payload.new.maintenance_enabled ?? false)
        setMaintStart(timeToHHMM(payload.new.maintenance_start) || '23:00')
        setMaintEnd(timeToHHMM(payload.new.maintenance_end) || '06:00')
      })
  ...
  async function issueDeviceCommand(command, payload = {}, label = 'Команда') {
    if (!door) return
    if (!door.device_id) {
      setTestMsg('error:Вратата няма зададено device_id. Изпълни SQL миграцията и задай doors.device_id.')
      return
    }
    setTestingDevice(true); setTestMsg('')
    const { data, error } = await supabase.from('device_commands').insert({
      door_id: door.id, command, payload, issued_by: currentUser.id, status: 'pending',
    }).select().single()
    if (error) {
      setTestMsg(`error:Грешка при изпращане: ${error.message}`)
    } else {
  ...
  const loadRecentCommands = useCallback(async (doorId) => {
    const { data } = await supabase.from('device_commands').select('*').eq('door_id', doorId).order('created_at', { ascending: false }).limit(10)
  ...
  async function saveMaintenance() {
    ...
    const { error } = await supabase.from('doors').update({
      ...
      maintenance_start: maintStart,
      maintenance_end: maintEnd,
    }).eq('id', door.id)
    if (!error) {
      await logAction('maintenance_updated', null, { enabled: maintEnabled, start: maintStart, end: maintEnd })
      setMaintSaved(true); setTimeout(() => setMaintSaved(false), 2000)
    }
    setMaintSaving(false)
  }
  async function generateQR() {
    const guestName = qrGuest.trim()
    if (!guestName || !qrFrom || !qrUntil) { await showAlert({ title: 'Непълни данни', message: 'Въведете име на госта и задайте период.', confirmLabel: 'Разбрах', tone: 'warning' }); return }
    ...
    const { data, error } = await supabase.from('temp_access').insert({ door_id: door.id, qr_code: qrCode, guest_name: guestName, valid_from: new Date(qrFrom).toISOString(), valid_until: new Date(qrUntil).toISOString(), created_by: currentUser.id, is_used: false }).select().single()
        ...
        {tab === 4 && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
            {!door ? (
              ...
              {/* ── Хардуер: device_id, heartbeat, тестови команди ─────────── */}
              <div style={{ background: 'var(--card-bg)', border: '1px solid var(--border)', borderRadius: 12, padding: '16px 20px' }}>
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 14, flexWrap: 'wrap', gap: 8 }}>
                  <div style={{ fontSize: 11, fontWeight: 500, color: 'var(--text-muted)', textTransform: 'uppercase', letterSpacing: 0.4 }}>Хардуер (ESP32)</div>
                  {(() => {
                    const hb = door.last_heartbeat ? new Date(door.last_heartbeat) : null
                    const ageMs = hb ? Date.now() - hb.getTime() : Infinity
                    const online = ageMs < 90000
                    return (
                      <div style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 11, color: online ? '#16a34a' : '#ef4444' }}>
                        <div style={{ width: 7, height: 7, borderRadius: '50%', background: online ? '#16a34a' : '#ef4444', boxShadow: online ? '0 0 6px #16a34a' : 'none' }} />
                        {online ? 'Онлайн' : (hb ? 'Офлайн' : 'Не е свързано')}
                      </div>
                    )
                  })()}
                </div>
                <div className="grid-2col" style={{ marginBottom: 14 }}>
                  <div>
                    <div style={{ fontSize: 11, color: 'var(--text-muted)', marginBottom: 3 }}>Device ID</div>
                    <div style={{ fontSize: 13, fontWeight: 500, color: 'var(--text)', fontFamily: 'monospace' }}>{door.device_id || <span style={{ color: '#ef4444' }}>не е зададено</span>}</div>
                  </div>
                  <div>
                    <div style={{ fontSize: 11, color: 'var(--text-muted)', marginBottom: 3 }}>Последен heartbeat</div>
                    <div style={{ fontSize: 13, fontWeight: 500, color: 'var(--text)' }}>{door.last_heartbeat ? formatDate(door.last_heartbeat) : '—'}</div>
                  </div>
                </div>
                {testMsg && (
                  <div style={{ fontSize: 12, padding: '8px 12px', borderRadius: 8, marginBottom: 12, ...(testMsg.startsWith('error:') ? { background: '#fef2f2', color: '#ef4444', border: '1px solid #fecaca' } : { background: '#f0fdf4', color: '#16a34a', border: '1px solid #bbf7d0' }) }}>
                    {testMsg.replace(/^(error|success):/, '')}
                  </div>
                )}
                <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', marginBottom: 14 }}>
                  <button onClick={() => issueDeviceCommand('test_led', { duration_ms: 2000 }, 'Тест LED')} disabled={testingDevice || !door.device_id} style={{ padding: '8px 16px', background: 'linear-gradient(135deg, #dd7fa2, #c9638b)', border: 'none', borderRadius: 8, color: '#fff', fontFamily: "'Inter', sans-serif", fontSize: 12, fontWeight: 600, cursor: (testingDevice || !door.device_id) ? 'not-allowed' : 'pointer', opacity: (testingDevice || !door.device_id) ? 0.6 : 1 }}>Тест LED (2s)</button>
                  <button onClick={() => issueDeviceCommand('unlock', { duration_ms: 3000 }, 'Отключване')} disabled={testingDevice || !door.device_id} style={{ padding: '8px 16px', background: 'var(--input-bg)', border: '1px solid var(--border)', borderRadius: 8, color: 'var(--text)', fontFamily: "'Inter', sans-serif", fontSize: 12, fontWeight: 500, cursor: (testingDevice || !door.device_id) ? 'not-allowed' : 'pointer', opacity: (testingDevice || !door.device_id) ? 0.6 : 1 }}>Отключи (3s)</button>
                  <button onClick={() => issueDeviceCommand('relock', {}, 'Затваряне')} disabled={testingDevice || !door.device_id} style={{ padding: '8px 16px', background: 'var(--input-bg)', border: '1px solid var(--border)', borderRadius: 8, color: 'var(--text)', fontFamily: "'Inter', sans-serif", fontSize: 12, fontWeight: 500, cursor: (testingDevice || !door.device_id) ? 'not-allowed' : 'pointer', opacity: (testingDevice || !door.device_id) ? 0.6 : 1 }}>Затвори</button>
                  <button onClick={() => issueDeviceCommand(door.is_locked ? 'emergency_unlock' : 'emergency_lock', {}, door.is_locked ? 'Аварийно отключване' : 'Аварийно заключване')} disabled={testingDevice || !door.device_id} style={{ padding: '8px 16px', background: door.is_locked ? '#f0fdf4' : '#fef2f2', border: `1px solid ${door.is_locked ? '#bbf7d0' : '#fecaca'}`, borderRadius: 8, color: door.is_locked ? '#16a34a' : '#ef4444', fontFamily: "'Inter', sans-serif", fontSize: 12, fontWeight: 500, cursor: (testingDevice || !door.device_id) ? 'not-allowed' : 'pointer', opacity: (testingDevice || !door.device_id) ? 0.6 : 1 }}>{door.is_locked ? 'Аварийно отключи' : 'Аварийно заключи'}</button>
                  <button onClick={() => issueDeviceCommand('reboot', {}, 'Рестарт')} disabled={testingDevice || !door.device_id} style={{ padding: '8px 16px', background: '#fef2f2', border: '1px solid #fecaca', borderRadius: 8, color: '#ef4444', fontFamily: "'Inter', sans-serif", fontSize: 12, fontWeight: 500, cursor: (testingDevice || !door.device_id) ? 'not-allowed' : 'pointer', opacity: (testingDevice || !door.device_id) ? 0.6 : 1 }}>Рестарт</button>
                </div>
                {recentCommands.length > 0 && (
                  <div>
                    <div style={{ fontSize: 11, fontWeight: 500, color: 'var(--text-muted)', textTransform: 'uppercase', letterSpacing: 0.4, marginBottom: 8 }}>Последни команди</div>
                    <div className="table-scroll">
                      <table style={{ width: '100%', fontSize: 12, borderCollapse: 'collapse' }}>
                        <thead>
                          <tr style={{ color: 'var(--text-muted)', textAlign: 'left' }}>
                            <th style={{ padding: '6px 8px', fontWeight: 500 }}>Команда</th>
                            <th style={{ padding: '6px 8px', fontWeight: 500 }}>Статус</th>
                            <th style={{ padding: '6px 8px', fontWeight: 500 }}>Време</th>
                          </tr>
                        </thead>
                        <tbody>
                          {recentCommands.map(c => (
                            <tr key={c.id} style={{ borderTop: '1px solid var(--border)' }}>
                              <td style={{ padding: '6px 8px', fontFamily: 'monospace', color: 'var(--text)' }}>{c.command}</td>
                              <td style={{ padding: '6px 8px' }}>
                                <span style={{ fontSize: 11, padding: '2px 8px', borderRadius: 20, ...(c.status === 'executed' ? { background: '#f0fdf4', color: '#16a34a', border: '1px solid #bbf7d0' } : c.status === 'sent' ? { background: '#eff6ff', color: '#2563eb', border: '1px solid #bfdbfe' } : c.status === 'failed' ? { background: '#fef2f2', color: '#ef4444', border: '1px solid #fecaca' } : { background: 'var(--input-bg)', color: 'var(--text-muted)', border: '1px solid var(--border)' }) }}>{c.status}</span>
                              </td>
                              <td style={{ padding: '6px 8px', color: 'var(--text-muted)' }}>{formatDate(c.created_at)}</td>
                            </tr>
                          ))}
                        </tbody>
                      </table>
              ...
              <div className="grid-2col" style={{ alignItems: 'start' }}>
                <div style={{ background: 'var(--card-bg)', border: `1px solid ${inMaintenance ? '#fde68a' : 'var(--border)'}`, borderRadius: 12, padding: '16px 20px' }}>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                    <button onClick={saveMaintenance} disabled={maintSaving} style={{ padding: '8px 18px', background: 'var(--input-bg)', border: '1px solid var(--border)', borderRadius: 8, fontFamily: "'Inter', sans-serif", fontSize: 13, fontWeight: 500, cursor: maintSaving ? 'not-allowed' : 'pointer', color: 'var(--text)', opacity: maintSaving ? 0.6 : 1 }}>{maintSaving ? 'Запазване...' : 'Запази'}</button>
                ...
                <div style={{ background: 'var(--card-bg)', border: '1px solid var(--border)', borderRadius: 12, padding: '16px 20px' }}>
                  ...
                  {qrLatest && (() => {
                    const guestUrl = `${window.location.origin}${window.location.pathname}#/guest/${qrLatest.qr_code}`
                    const qrSrc = `https://api.qrserver.com/v1/create-qr-code/?size=180x180&data=${encodeURIComponent(guestUrl)}`
                    return (
                      <div style={{ padding: '14px', background: '#f0fdf4', border: '1px solid #bbf7d0', borderRadius: 10, display: 'flex', flexDirection: 'column', gap: 10 }}>
                        <div style={{ display: 'flex', gap: 14, alignItems: 'center', flexWrap: 'wrap' }}>
                          <img src={qrSrc} alt="QR" style={{ width: 120, height: 120, borderRadius: 6, border: '2px solid #fff', flexShrink: 0, background: '#fff' }} />
                        ...
                        <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
                          <input readOnly value={guestUrl} style={{ flex: 1, minWidth: 180, padding: '6px 10px', fontSize: 11, fontFamily: 'monospace', background: '#fff', border: '1px solid #bbf7d0', borderRadius: 6, color: '#166534' }} />
                          <button onClick={() => { navigator.clipboard?.writeText(guestUrl) }} style={{ padding: '6px 10px', fontSize: 11, fontWeight: 500, background: '#16a34a', color: '#fff', border: 'none', borderRadius: 6, cursor: 'pointer' }}>Копирай</button>
                          <a href={qrSrc} download={`qr-${getTempAccessGuestName(qrLatest)}.png`} style={{ padding: '6px 10px', fontSize: 11, fontWeight: 500, background: '#fff', color: '#16a34a', border: '1px solid #bbf7d0', borderRadius: 6, textDecoration: 'none' }}>Свали PNG</a>
                        </div>
                      </div>
                    )
                  })()}
