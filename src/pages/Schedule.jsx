...
function ScheduleBadge({ state }) {
  return <span style={{ fontSize: 11, fontWeight: 500, padding: '3px 8px', borderRadius: 20, ...statusStyles[state] }}>{statusLabels[state]}</span>
}
    ...
    <Layout>
      <main className="page-main" style={{ background: 'var(--bg)', minHeight: 'calc(100vh - 56px)' }}>
        <div style={{ marginBottom: 24 }}>
          ...
          <div style={{ fontSize: 13, color: 'var(--text-muted)' }}>Само администратори могат да управляват графика.</div>
        </div>
        ...
        {error && <div style={{ fontSize: 12, color: '#ef4444', padding: '10px 12px', background: '#fef2f2', border: '1px solid #fecaca', borderRadius: 10, marginBottom: 16 }}>{error}</div>}
        {success && <div style={{ fontSize: 12, color: '#16a34a', padding: '10px 12px', background: '#f0fdf4', border: '1px solid #bbf7d0', borderRadius: 10, marginBottom: 16 }}>{success}</div>}

        <div className="grid-fixed">

          <section style={{ background: 'var(--card-bg)', border: '1px solid var(--border)', borderRadius: 12, padding: '20px 20px' }}>
            <div style={{ fontSize: 14, fontWeight: 600, color: 'var(--text)', marginBottom: 4 }}>Нов график</div>
            <div style={{ fontSize: 12, color: 'var(--text-muted)', marginBottom: 16, lineHeight: 1.5 }}>Задайте за кои дни и часове потребителят може да управлява вратата.</div>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
              <Field label="Потребител">
                <select style={inp} value={form.userId} onChange={e => setForm(c => ({ ...c, userId: e.target.value }))} disabled={activeUsers.length === 0}>
                  ...
                  {DAY_OPTIONS.map(day => {
                    ...
                    return (
                      <button key={day.value} type="button" onClick={() => setForm(c => ({ ...c, days: toggleDay(c.days, day.value) }))}
                        style={{ padding: '9px 0', borderRadius: 8, border: selected ? '1px solid transparent' : '1px solid var(--border)', background: selected ? 'var(--btn-bg)' : 'var(--card-bg)', color: selected ? 'var(--btn-color)' : 'var(--text)', fontFamily: "'Inter', sans-serif", fontSize: 12, fontWeight: 600, cursor: 'pointer' }}>
                        {day.shortLabel}
                      </button>
                    )
                  })}
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
                <Field label="Начало"><input type="time" style={inp} value={form.openTime} onChange={e => setForm(c => ({ ...c, openTime: e.target.value }))} /></Field>
                <Field label="Край"><input type="time" style={inp} value={form.closeTime} onChange={e => setForm(c => ({ ...c, closeTime: e.target.value }))} /></Field>
              </div>
              <button style={{ width: '100%', padding: 10, background: 'var(--btn-bg)', border: 'none', borderRadius: 8, color: 'var(--btn-color)', fontFamily: "'Inter', sans-serif", fontSize: 13, fontWeight: 500, cursor: activeUsers.length === 0 ? 'not-allowed' : 'pointer', opacity: activeUsers.length === 0 ? 0.5 : 1 }}
          ...
          <section style={{ background: 'var(--card-bg)', border: '1px solid var(--border)', borderRadius: 12, overflow: 'hidden' }}>
            <div style={{ padding: '16px 20px', borderBottom: '1px solid var(--border)' }}>
              ...
              <div style={{ fontSize: 12, color: 'var(--text-muted)' }}>{decoratedSchedules.length === 0 ? 'Все още няма създадени графици.' : `${decoratedSchedules.length} графика общо`}</div>
            </div>
            <div style={{ padding: 16, display: 'flex', flexDirection: 'column', gap: 10 }}>
              {decoratedSchedules.length === 0 ? (
                <div style={{ padding: '24px 10px', textAlign: 'center', color: 'var(--text-muted)', fontSize: 13 }}>Няма създадени графици.</div>
                  ...
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: 12, flexWrap: 'wrap' }}>
                    <div style={{ minWidth: 0, flex: 1 }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap', marginBottom: 6 }}>
                        <div style={{ fontSize: 13, fontWeight: 600, color: 'var(--text)' }}>{schedule.user?.first_name} {schedule.user?.last_name}</div>
                        <ScheduleBadge state={schedule.state} />
                      </div>
                      <div style={{ fontSize: 12, color: 'var(--text-muted)', marginBottom: 6 }}>{schedule.user?.email || 'Няма имейл'}</div>
                      <div style={{ fontSize: 13, color: 'var(--text)', marginBottom: 6, lineHeight: 1.5 }}>{formatScheduleRule(schedule)}</div>
                      {schedule.currentWindow && <div style={{ fontSize: 12, color: '#166534', marginBottom: 6 }}>Активен прозорец: {formatAccessWindowShort(schedule.currentWindow.start, schedule.currentWindow.end)}</div>}
                      {!schedule.currentWindow && schedule.nextWindow && (
                        <>
                          <div style={{ fontSize: 12, color: '#92400e', marginBottom: 4 }}>Следващ: {formatAccessWindowShort(schedule.nextWindow.start, schedule.nextWindow.end)}</div>
                          <div style={{ fontSize: 11, color: '#92400e', marginBottom: 6 }}>{formatCountdown(schedule.nextWindow.start)}</div>
                        </>
                      )}
                      {schedule.state === 'inactive' && <div style={{ fontSize: 12, color: 'var(--text-muted)', marginBottom: 6 }}>Не дава активен достъп в момента.</div>}
                    </div>
                    <button onClick={() => deleteSchedule(schedule.id)} disabled={deletingId === schedule.id}
                      style={{ padding: '7px 10px', background: '#fef2f2', border: '1px solid #fecaca', borderRadius: 8, color: '#ef4444', fontFamily: "'Inter', sans-serif", fontSize: 12, fontWeight: 500, cursor: 'pointer', whiteSpace: 'nowrap', flexShrink: 0 }}>
                      {deletingId === schedule.id ? 'Премахване...' : 'Премахни'}
                    </button>
                  </div>
                  <div style={{ marginTop: 10, fontSize: 11, color: 'var(--text-light)' }}>Кратък преглед: {formatScheduleRule(schedule, true)}</div>
                </div>
              ))}
            </div>
          </section>
